...
class WC_Boacompra_My_Account {
	...
	public function legacy_my_orders_bank_slip_link( $actions, $order ) {
		if ( 'boacompra-bank-slip' !== $order->get_payment_method() ) {
			return $actions;
		}
		...
		if ( ! empty( $data['pdf'] ) ) {
			$actions[] = array(
				'url'  => $data['pdf'],
				'name' => __( 'Pay the Bank Slip', 'boacompra-woocommerce' ),
			);
		}
