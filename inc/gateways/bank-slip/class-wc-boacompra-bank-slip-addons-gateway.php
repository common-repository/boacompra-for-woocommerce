...
/**
 * BoaCompra Payment Bank Slip Addons Gateway class.
 *
 * Integration with WooCommerce Subscriptions.
 *
 * @class   WC_Boacompra_Bank_Slip_Addons_Gateway
 * @extends WC_Boacompra_Bank_Slip_Gateway
 *
 * @version 2.0.0
 * @author  BoaCompra
 */
class WC_Boacompra_Bank_Slip_Addons_Gateway extends WC_Boacompra_Bank_Slip_Gateway {
	...
	public function __construct() {
		...
		add_action('woocommerce_api_wc_boacompra_bank_slip_addons_gateway', array( $this->api, 'notification_handler' ));
	...
	public function process_subscription_payment($order = '', $amount = 0) {
		...
		if (isset($charge['errors']) && !empty($charge['errors'])) {
			...
			return new WP_Error( 'boacompra_subscription_error', $charge['errors']);

		} // end if;

		update_post_meta($order->get_id(), '_boacompra_wc_transaction_data', $charge['transactions']);

		update_post_meta($order->get_id(), '_boacompra_wc_transaction_id', $charge['transactions']['code']);

		update_post_meta( $order->get_id(), __('BoaCompra Bank Slip: URL', 'boacompra-woocommerce'), $charge['transactions']['payment-url']);

		$order_note = __('BoaCompra: The customer generated a bank slip. Awaiting payment confirmation.', 'boacompra-woocommerce');
	...
	protected function update_subscription_status($order_id, $invoice_status) {
		...
		if ($invoice_status == 'paid' || $invoice_status == 'PAID') {
			...
			$order->add_order_note( __( 'BoaCompra: Subscription paid successfully.', 'boacompra-woocommerce' ) );
			...
			$order->add_order_note(__('BoaCompra: Subscription payment failed.', 'boacompra-woocommerce'));
		 ...
		 */
		do_action( 'boacompra_woocommerce_update_order_status', $order, $invoice_status, $order_updated );
