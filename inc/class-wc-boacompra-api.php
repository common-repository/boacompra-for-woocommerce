...
/**
 * WC BoaCompra API Class.
 */
class WC_Boacompra_API {
	 ...
	 * Constructor.
	 *
	 * @param WC_Boacompra_Gateway $gateway
	 */
	...
	public function process_payment($order_id) {
		...
		$payload = $this->get_boacompra_payload($order, $_POST);
		...
		if ($this->method === 'pix') {
			...
			return array(
				'result'   => 'success',
				'redirect' => get_site_url().'/wc-api/wc_boacompra_hosted_pix_request?oid=' . $order_id,
			);
		...
		if ($charge['transaction']) {
			...
			update_post_meta($order->get_id(), '_boacompra_wc_transaction_data', $charge['transaction']);

			update_post_meta($order->get_id(), '_boacompra_wc_transaction_id', $charge['transaction']['code']);
	...
	public function process_refund($order_id, $amount) {
		...
		$transaction_id = get_post_meta($order_id, '_boacompra_wc_transaction_id');
	 ...
	 * @param string $status BoaCompra Status
	 * @return bool
	 */
	protected function update_order_status($order_id, $status) {
		...
		switch ($check_status) {
			case 'CANCELLED':
				$message = __('Transaction was cancelled by UOL BoaCompra', 'boacompra-woocommerce');
			...
			case 'COMPLETE':
				$message = __('Transaction was paid and approved.', 'boacompra-woocommerce');
			...
			case 'CHARGEBACK':
				$message = __('An approved transaction was cancelled by the End User. Please consult your Account Manager for costs.', 'boacompra-woocommerce');
			...
			case 'EXPIRED':
				$message = __('Payment date of transaction expired', 'boacompra-woocommerce');
			...
			case 'PENDING':
				...
				if ($this->method === 'bank-slip') {
					...
					$message = __('BoaCompra: The customer generated a bank slip. Awaiting payment confirmation.', 'boacompra-woocommerce');

					$order->update_status('on-hold', $message);

				} else {

					$message = __('BoaCompra: Invoice paid. Waiting for the acquirer confirmation.', 'boacompra-woocommerce');
				...
				break;
			case 'REFUNDED':
				$message = __('A partial or full refund was requested and accepted for the transaction', 'boacompra-woocommerce');
				...
				$this->send_email(
					sprintf(__( 'Invoice for order %s was refunded', 'boacompra-woocommerce' ), $order->get_order_number()),
					__('Invoice refunded', 'boacompra-woocommerce'),
					sprintf(__('Order %s has been marked as refunded by boacompra.', 'boacompra-woocommerce'), $order->get_order_number())
				);
		...
		if ($this->gateway->debug === 'yes') {
			...
			$this->gateway->log->add($this->gateway->id, 'boacompra payment status for order ' . $order->get_order_number() . ' is now: ' . $message);
		 ...
		 */
		do_action( 'boacompra_woocommerce_update_order_status', $order, $status, $message);
	...
	public function notification_handler() {
		...
		wp_die(__('The request failed!', 'boacompra-woocommerce' ), __('The request failed!', 'boacompra-woocommerce' ), array('response' => 200));
	...
	public function refund_order($order_id, $amount) {
		...
		if($total != $amount) throw new Exception( __( "Can't do partial refunds", 'boacompra-woocommerce' ) );
	...
	/**
	 * Get BoaCompra installments based on the card brand.
	 *
	 * @since 2.0.0
	 *
	 ...
	 * @return mixed.
	 */
	public function get_boacompra_installments($payload) {
		...
		if (is_array($payload)) {
			...
			foreach($response['installments'] as $key_installment => $value_installment) {
				...
				if ($value_installment['quantity'] > $max_installments) {
					...
					$installments[$value_installment['quantity']] = '<option value="' . $value_installment['quantity'] . '">' . sprintf(__('%1s x %2s (Total: %3s)', 'boacompra-woocommerce'), $value_installment['quantity'], $installment_amount, $total_amount) . '</option>';
	...
	} // end get_boacompra_payload_redirect;
	...
	public function get_country_language($country) {
 	...
 	/**
		...
		*/
	public function get_boacompra_payload($order) {
	...
	} // end get_boacompra_payload;

	/**
	 * Create the section Transaction in the Boacompra payload.
	 *
	 * @since 2.0.0
	 *
	 * @param WC_Order $order WooCommerce Order.
	...
	/**
	 * Create the section Charge in the Boacompra payload.
	 *
	 * @since 2.0.0
	 *
	 * @param WC_Order $order WooCommerce Order.
	...
	public function get_payload_charge($order) {
		...
		switch ($this->method) {
			case 'credit-card':
				...
				$charge['payment-method']['sub-type'] = wcbc_request('boacompra_card_brand', '');;

				$charge['payment-info']['installments'] = (int) wcbc_request('boacompra_card_installments', 1);

				$charge['payment-info']['token'] = wcbc_request('boacompra_card_token', '');

				if (!$charge['payment-info']['token'] || !$charge['payment-method']['sub-type']) {

					wc_add_notice(__('Your credit card data is invalid. Try again!.', 'boacompra-woocommerce'), 'error');
			...
			case 'e-wallet':
				...
				$charge['payment-method']['sub-type'] = wcbc_request('boacompra_wallet', 'pagseguro');
	...
	/**
	 * Create the section payer in the Boacompra payload.
	 *
	 * @since 2.0.0
	 *
	 * @param WC_Order $order WooCommerce Order.
		...
		foreach ($payer['address'] as $key_address => $value_address) {
			...
			if (!$value_address && $key_address !== 'complement' ) {
				...
				if ($key_address === 'district') {
					...
					$field = __('Neighborhood', 'boacompra-woocommerce');

				} // end if;

				$message = sprintf(__('%s invalid.', 'boacompra-woocommerce'), $field);
	...
	/**
	 * Create the section shippíng in the Boacompra payload.
	 *
	 * @since 2.0.0
	 *
	 * @param WC_Order $order WooCommerce Order.
	...
	/**
	 * Do requests in the BoaCompra API.
	 *
	...
	public function get_invoice_status($transaction_id = '', $is_refund = false) {
		...
		if (isset($request) && isset($request['errors'])) {
			...
			if ($this->gateway->debug == 'yes') {
				...
				$this->gateway->log->add( $this->gateway->id, 'Error: Getting invoice status from BoaCompra. Invoice ID: ' . $transaction_id );
   ...
   * @return string
   */
  public function get_client_ip($onlyipv4 = false) {
